/** @OnlyCurrentDoc */
function setupClass10IT() {
 const ss=SpreadsheetApp.getActiveSpreadsheet();
 if(ss.getId()!=="1gJNQ1OSCsaStvwC11MLoapPxwqNYcUdcgyYVy5qUhEo") throw new Error("Chỉ chạy trong Sheet lớp 10IT.");
 ss.setSpreadsheetTimeZone("Asia/Ho_Chi_Minh");
 const tabs={HOC_SINH:["MaHS","HoTen","SDTPHHS","MaTraCuu","KhoiThi","TongDiemMucTieu"],BANG_DIEM_WEB:["MaHS","HoTen"],NHAN_XET:["MaHS","HoTen"],DIEM_DANH:["Ngay","MaHS","TrangThai","CapNhatLuc"],BTVN:homeworkHeaders(),BTVN_GIAO_BAI:["SuKienJSON"],CAI_DAT_WEB:["Khoa","GiaTri","CapNhatLuc"]};
 Object.keys(tabs).forEach(name=>{
   let s=ss.getSheetByName(name);
   if(!s) {const first=ss.getSheets()[0];s=ss.getSheets().length===1&&first.getLastRow()===0?first.setName(name):ss.insertSheet(name);}
   if(s.getLastRow()===0){s.getRange(1,1,1,tabs[name].length).setValues([tabs[name]]).setFontWeight("bold").setBackground("#e8eefc");s.setFrozenRows(name==="BANG_DIEM_WEB"?5:1);s.getRange("A:A").setNumberFormat("@");s.autoResizeColumns(1,tabs[name].length);}
 });
 const students=ss.getSheetByName("HOC_SINH");students.getRange("C:D").setNumberFormat("@");
 const scores=ss.getSheetByName("BANG_DIEM_WEB");if(scores.getLastRow()<2) scores.getRange(2,1).setValue("Mã bài ở hàng 3 (TOAN_BAI_01), tên bài hàng 4, ngày hàng 5; điểm từ hàng 6. Có thể tạo bài trực tiếp trên web.");
 const settings=ss.getSheetByName("CAI_DAT_WEB");if(settings.getLastRow()<2)settings.getRange(2,1,2,3).setValues([["className","LỚP 10IT",new Date().toISOString()],["academicYear","2026 - 2027",new Date().toISOString()]]);
 SpreadsheetApp.flush();console.log("Đã chuẩn bị lớp 10IT, chưa có dữ liệu học sinh.");
}
...
function doGet(e) {
  ...
  try {
    ...
    if (mode === "score-archive" || mode === "score-restore") return output(scoreArchiveAction(parameters), callback);
...
function readScoreBookForAdmin(ss) {
  ...
  if (values.length >= 5) {
    ...
    for (let column = 2; column < codes.length; column++) {
      ...
      tests.push({
        code: code,
        revision: scoreTestRevision(values, notes, column),
        subjectCode: match[1],
  ...
  return {
    deletedTests: (values[2] || []).map((code,col) => ({code:String(code).replace(/^ARCHIVED_/, ""), title:String((values[3]||[])[col]||""), archived:String(code).indexOf("ARCHIVED_")===0})).filter(t=>t.archived),
    tests: tests,
    students: rows
  };
}
...
function saveScoreBookForAdmin(ss, input) {
  ...
  try {
    ...
    if (testCode) {
      ...
      if (!targetColumn) {
        if(testInput.recoverMissing !== true) throw new Error("Không tìm thấy bài kiểm tra cần cập nhật.");
        if(!new RegExp("^"+subjectCode+"_BAI_\\d+$").test(testCode)) throw new Error("Mã bài khôi phục không hợp lệ.");
        if(codes.some(v=>String(v).trim().toUpperCase()==="ARCHIVED_"+testCode)) throw new Error("Bài đã xóa. Hãy dùng Khôi phục trong mục Bài đã xóa.");
        targetColumn=Math.max(sheet.getLastColumn()+1,3);
      }
      if (!testInput.recoverMissing && testInput.revision && testInput.revision !== scoreTestRevision(sheet.getDataRange().getDisplayValues(),sheet.getDataRange().getNotes(),targetColumn-1)) throw new Error("Điểm trên Sheet đã thay đổi. Bản nhập trên máy vẫn được giữ; hãy sao lưu rồi tải điểm mới nhất để đối chiếu.");
    } else {
      let nextNumber = 1;
      codes.forEach(value => {
        const match = String(value || "").trim().toUpperCase().replace(/^ARCHIVED_/, "")
    ...
    (Array.isArray(input.scores)?input.scores:[]).forEach(item=>{const raw=String(item.score??"").trim().replace(",", "."); if(raw && (!Number.isFinite(Number(raw)) || Number(raw)<0 || Number(raw)>10)) throw new Error("Điểm phải nằm trong khoảng 0 đến 10.");});
    if(targetColumn>sheet.getMaxColumns()) sheet.insertColumnsAfter(sheet.getMaxColumns(),targetColumn-sheet.getMaxColumns());
...
function hwPublishConfig(){return JSON.parse(PropertiesService.getScriptProperties().getProperty('HW_PUBLISHER_V1')||'{}');}
function hwPublishSheet(name){return SpreadsheetApp.getActiveSpreadsheet().getSheetByName(name);}
function hwPublishEvents(){var sheet=hwPublishSheet('BTVN_GIAO_BAI');if(!sheet||sheet.getLastRow()<2)return [];return sheet.getRange(2,1,sheet.getLastRow()-1,1).getValues().map(function(row){return JSON.parse(row[0]);});}
function hwPublishAppend(event){var sheet=hwPublishSheet('BTVN_GIAO_BAI');if(!sheet){sheet=SpreadsheetApp.getActiveSpreadsheet().insertSheet('BTVN_GIAO_BAI');sheet.getRange(1,1).setValue('SuKienJSON');}sheet.appendRow([JSON.stringify(event)]);}
function hwPublishLatest(events){var latest={};events.forEach(function(event){if(event.post)latest[event.post.id]=event.post;});return Object.keys(latest).map(function(id){return latest[id];});}
function hwPublishHandle(p){
 var mode=String(p.mode||''),lock=LockService.getScriptLock();if(!lock.tryLock(10000))throw new Error('Đang có thao tác khác. Vui lòng thử lại.');
 try{
 var props=PropertiesService.getScriptProperties(),config=hwPublishConfig();var admin=mode.indexOf('hw-admin-')===0;if(admin)assertAdminKey(p.adminKey);
 if(mode==='hw-admin-info')return {ok:true,publisher:config.studentCode?{studentCode:config.studentCode,name:config.name}:null,students:getClassList().map(function(s){return {studentCode:s.studentCode,name:s.name};})};
 if(mode==='hw-admin-grant'){
 var student=getClassList().filter(function(s){return String(s.studentCode)===String(p.studentCode);})[0];if(!student)throw new Error('Hãy chọn học sinh trong danh sách lớp.');var pin=String(p.pin||'');if(!/^[A-Za-z0-9]{8,32}$/.test(pin))throw new Error('Mã riêng cần 8–32 chữ hoặc số.');if(pin===String(p.adminKey))throw new Error('Mã học sinh phải khác mã quản trị.');var version=Utilities.getUuid();props.setProperty('HW_PUBLISHER_V1',JSON.stringify({studentCode:String(student.studentCode),name:student.name,version:version,pinHash:hwPublishHash(version+':'+pin)}));return {ok:true};}
 if(mode==='hw-admin-revoke'){props.deleteProperty('HW_PUBLISHER_V1');return {ok:true};}
 if(mode==='hw-login'){
 var cache=CacheService.getScriptCache(),failKey='hw_login_fail_v1',failures=Number(cache.get(failKey)||0);if(failures>=10)throw new Error('Có nhiều lượt nhập sai. Vui lòng chờ 10 phút hoặc liên hệ GVCN.');if(!config.version||hwPublishHash(config.version+':'+String(p.pin||''))!==config.pinHash){cache.put(failKey,String(failures+1),600);throw new Error('Mã không đúng hoặc quyền đã bị thu hồi.');}cache.remove(failKey);var token=Utilities.getUuid()+Utilities.getUuid();config.tokenHash=hwPublishHash(token);config.expiresAt=Date.now()+30*86400000;props.setProperty('HW_PUBLISHER_V1',JSON.stringify(config));return {ok:true,token:token,name:config.name,studentCode:config.studentCode,expiresAt:config.expiresAt};}
 var studentMode=['hw-session','hw-save','hw-logout'].indexOf(mode)>=0;
 if(studentMode&&(!config.tokenHash||!p.token||hwPublishHash(p.token)!==config.tokenHash||Date.now()>=config.expiresAt))return {ok:false,authExpired:true,message:'Phiên đăng nhập hết hạn hoặc quyền đã bị thu hồi. Hãy đăng nhập lại.'};
 if(mode==='hw-session')return {ok:true,name:config.name,studentCode:config.studentCode};
 if(mode==='hw-logout'){delete config.tokenHash;delete config.expiresAt;props.setProperty('HW_PUBLISHER_V1',JSON.stringify(config));return {ok:true};}
 if(['hw-list','hw-admin-list','hw-admin-history','hw-save','hw-admin-save','hw-admin-restore'].indexOf(mode)<0)throw new Error('Thao tác không được hỗ trợ.');
 ...
 if(mode==='hw-list'||mode==='hw-admin-list')return {ok:true,posts:posts.filter(function(post){return admin||!post.deleted;}).map(function(post){var out=Object.assign({},post);if(!admin)delete out.authorName;return out;}).sort(function(a,b){return b.updatedAt.localeCompare(a.updatedAt);})};
 var input=JSON.parse(String(p.post||'{}')),previous;
 if(mode==='hw-admin-restore'){var prior=events.filter(function(e){return e.eventId===String(p.eventId);})[0];if(!prior)throw new Error('Không tìm thấy phiên bản.');input=Object.assign({},prior.post,{deleted:false});previous=posts.filter(function(post){return post.id===input.id;})[0];input.baseRevision=previous&&previous.revision;}else previous=posts.filter(function(post){return post.id===String(input.id||'');})[0];
 if(!admin&&previous&&(previous.authorCode!==config.studentCode||previous.deleted))throw new Error('Bạn chỉ được sửa bài mình đăng và chưa bị GVCN gỡ.');
 ...
 if(!/^\d{4}-\d{2}-\d{2}$/.test(due)||isNaN(Date.parse(due+'T00:00:00Z'))||new Date(due+'T00:00:00Z').toISOString().slice(0,10)!==due)throw new Error('Chọn hạn nộp hợp lệ.');
 var post={id:previous?previous.id:String(input.id||Utilities.getUuid()),subject:subject,content:content,due:due,authorCode:previous?previous.authorCode:(admin?'GVCN':config.studentCode),authorName:previous?previous.authorName:(admin?'GVCN':config.name),createdAt:previous?previous.createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),revision:Utilities.getUuid(),deleted:admin?Boolean(input.deleted):false};
 hwPublishAppend({eventId:Utilities.getUuid(),actor:admin?'GVCN':config.name,action:mode,post:post});return {ok:true,post:post};
 }finally{lock.releaseLock();}
}

function scoreTestRevision(values, notes, column) {
  return Utilities.base64EncodeWebSafe(Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, JSON.stringify(values.map((row,i)=>[row[0],row[column],(notes[i]||[])[column]]))));
}
function scoreArchiveAction(parameters) {
  assertAdminKey(parameters.adminKey);
  const lock=LockService.getScriptLock(); lock.waitLock(20000);
  try {
    const ss=SpreadsheetApp.getActiveSpreadsheet(), sheet=ss.getSheetByName("BANG_DIEM_WEB");
    if(!sheet) throw new Error("Không tìm thấy bảng điểm.");
    const code=String(parameters.testCode||"").trim().toUpperCase();
    if(!/^(TOAN|VAN|ANH|LY|HOA|SINH)_BAI_\d+$/.test(code)) throw new Error("Mã bài kiểm tra không hợp lệ.");
    const restore=parameters.mode==="score-restore", values=sheet.getDataRange().getDisplayValues();
    const from=restore?"ARCHIVED_"+code:code, to=restore?code:"ARCHIVED_"+code;
    const col=(values[2]||[]).findIndex(v=>String(v).trim().toUpperCase()===from);
    if(col<2) throw new Error("Bài đã thay đổi. Hãy tải lại danh sách.");
    if((values[2]||[]).some(v=>String(v).trim().toUpperCase()===to)) throw new Error("Mã bài bị trùng, cần kiểm tra Sheet.");
    if(!restore && parameters.revision && parameters.revision!==scoreTestRevision(values,sheet.getDataRange().getNotes(),col)) throw new Error("Điểm trên Sheet đã thay đổi. Hãy tải lại trước khi xóa.");
    sheet.getRange(3,col+1).setValue(to); SpreadsheetApp.flush();
    return {ok:true,updatedAt:new Date().toISOString(),scoreBook:readScoreBookForAdmin(ss)};
  } finally {lock.releaseLock();}
}
